...
export const FillBlankQ: React.FC<Props> = ({ question, onAnswer }) => {
        ...
        {(question.options || []).map((option, index) => {
          ...
          return (
            <Button
              ...
              variant="outline"
              className={cn(
                'h-auto py-4 px-4 text-center justify-center text-base font-normal whitespace-normal break-words',
            ...
            >
              <div className="flex items-center gap-2">
                <span>{option}</span>
                {showResult && isCorrectOption && <CheckCircle2 className="w-4 h-4 text-green-600 dark:text-green-400 shrink-0" />}
                {showResult && isSelected && !isCorrectOption && <XCircle className="w-4 h-4 text-destructive shrink-0" />}
              </div>
            </Button>
          );
        })}
